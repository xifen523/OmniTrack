...
class InstanceBankJRDB2D(nn.Module):
    def __init__(
        self,
        num_anchor,
        embed_dims,
        ...
        super(InstanceBankJRDB2D, self).__init__()
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        if (
            ...
            if self.anchor_handler is not None:   # project to current frame
                # NOTE: the projection is done in the previous frame, this should be fixed in the future(best innovation)
                if "T_global_inv" in metas["img_metas"][0] and "T_global" in self.metas["img_metas"][0]:
            ...
            if (
                ...
                if T_temp2cur is not None:
                    ...
                    dn_metas["dn_anchor"] = dn_metas["dn_anchor"].reshape(
                        batch_size, num_dn_group, num_dn, -1
                    )
