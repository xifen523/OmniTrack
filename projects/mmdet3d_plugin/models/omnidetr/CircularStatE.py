...
class CircularStatE(nn.Module):
    ...
    def __init__(
        ...
        # self.emd_1 = ring_mamba.DeformableMambaProjEmbed(in_chans=in_channels[0], emb_chans=in_channels[0])
        self.emd_2 = DynamicSSM.DeformableMambaProjEmbed(in_chans=in_channels[1], emb_chans=in_channels[1])
        # self.emd_3 = ring_mamba.DeformableMambaProjEmbed(in_chans=in_channels[2], emb_chans=in_channels[2])

        
    

    def forward(self, x, batch=None):
        # y = [self.emd_1(x[0]), x[1], x[2]]
        y = [x[0], self.emd_2(x[1]), x[2]]
        # y = [x[0], x[1], x[2]]
        outputs_features = []
