...
__all__ = ["OmniDETRBox2DTarget"]


@BBOX_SAMPLERS.register_module()
class OmniDETRBox2DTarget(BaseTargetWithDenoising):
    def __init__(
        self,
        cls_weight=2.0,
        alpha=0.25,
        ...
        super(OmniDETRBox2DTarget, self).__init__(
    ...
    def _box_cost(self, box_pred, box_target, instance_reg_weights):
        ...
        for i in range(bs):
            if len(box_target[i]) > 0:
                ...
                # # nan or inf in box_target will cause nan or inf in cost, replace it with 100
                # cost[-1] = torch.where(
                #     torch.isinf(cost[-1]) | torch.isnan(cost[-1]),
                #     cost[-1].new_tensor(100),
                #     cost[-1],
                # )
            else:
                cost.append(None)
    ...
    def get_dn_anchors(self, cls_target, box_target, gt_instance_id=None):
        ...
        )   # pad with zeros for negative samples
        if gt_instance_id is not None:
        ...
        if self.num_dn_groups > 1:
            cls_target = cls_target.tile(self.num_dn_groups, 1)  # repeat for groups
        ...
        for i in range(dn_anchor.shape[0]):    # 匈牙利匹配 指定正负样本
