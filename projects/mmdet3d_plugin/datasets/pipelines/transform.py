...
@PIPELINES.register_module()
class JRDBSparse4DAdaptor(object):
    def __init(self):
        pass
...
@PIPELINES.register_module()
class CircleObjectRangeFilterJRDB2D(object):
    def __init__(
        self, class_arce_thred=[52.5] * 5 + [31.5] + [42] * 3 + [31.5]
    ):
        self.class_arce_thred = class_arce_thred

    def __call__(self, input_dict):
        gt_bboxes_2d = input_dict["gt_bboxes_2d"]
        gt_labels_2d = input_dict["gt_labels_2d"]
        dist = np.sqrt(
            np.sum(gt_bboxes_2d[:, 2:4] ** 2, axis=-1)
        )
        mask = np.array([False] * len(dist))
        for label_idx, dist_thred in enumerate(self.class_arce_thred):
            mask = np.logical_or(
                mask,
                np.logical_and(gt_labels_2d == label_idx, dist >= dist_thred),
            )

        gt_bboxes_2d = gt_bboxes_2d[mask]
        gt_labels_2d = gt_labels_2d[mask]

        input_dict["gt_bboxes_2d"] = gt_bboxes_2d
        input_dict["gt_labels_2d"] = gt_labels_2d
        if "instance_inds" in input_dict:
            input_dict["instance_inds"] = input_dict["instance_inds"][mask]
...
@PIPELINES.register_module()
class InstanceNameFilterJRDB2D(object):
    """Filter GT objects by their names.
    ...
    def __call__(self, input_dict):
        ...
        Returns:
            dict: Results after filtering, 'gt_bboxes_2d', 'gt_labels_2d' \
                keys are updated in the result dict.
        """
        gt_labels_2d = input_dict["gt_labels_2d"]
        gt_bboxes_mask = np.array(
            [n in self.labels for n in gt_labels_2d], dtype=np.bool_
        )
        input_dict["gt_bboxes_2d"] = input_dict["gt_bboxes_2d"][gt_bboxes_mask]
        input_dict["gt_labels_2d"] = input_dict["gt_labels_2d"][gt_bboxes_mask]
        if "instance_inds" in input_dict:
            input_dict["instance_inds"] = input_dict["instance_inds"][
                gt_bboxes_mask
            ]
...
@PIPELINES.register_module()
class JRDBSparse2DAdaptor(object):
    def __init(self):
        pass
    ...
    def __call__(self, input_dict):
        ...
        if "gt_bboxes_2d" in input_dict:
            ...
            input_dict["gt_bboxes_2d"] = DC(
                to_tensor(input_dict["gt_bboxes_2d"]).float()
            )
        if "gt_labels_2d" in input_dict:
            input_dict["gt_labels_2d"] = DC(
                to_tensor(input_dict["gt_labels_2d"]).long()
            )
