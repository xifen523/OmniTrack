...
class Box:
    ...
    def __init__(self,
        ...
        self.yaw = float(yaw) if not np.isnan(yaw) else yaw
        self.label = int(label) if not np.isnan(label) else label
        self.score = float(score) if not np.isnan(score) else score
        self.cls_score = float(cls_score) if not np.isnan(cls_score) else cls_score  # for compatibility with other classes
    ...
    def __repr__(self):
        repr_str = 'label: {}, score: {:.2f}, xyz: [{:.2f}, {:.2f}, {:.2f}], wlh: [{:.2f}, {:.2f}, {:.2f}], ' \
                   'yaw: {:.2f} ang(rad): {:.2f}, vel: {:.2f}, {:.2f}, {:.2f}, name: {}, token: {} cls_score: {:.2f}'

        return repr_str.format(self.label, self.score, self.center[0], self.center[1], self.center[2], self.wlh[0],
                               self.wlh[1], self.wlh[2], self.yaw, self.velocity[0], self.velocity[1], self.velocity[2],
                               self.name, self.token, self.cls_score)
